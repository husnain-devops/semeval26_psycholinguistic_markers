...
# SemEval 2026 Task 10 — Marker Extraction (Evidence)

RoBERTa-large with LoRA for **Evidence** span extraction (SemEval 2026 Task 10, marker extraction subtask).

## Marker: Evidence

Spans denoting **supporting information or proof** (e.g. articles, data, documents, testimony) cited in the text.

## Model description

- **Task:** Token-level binary classification for Evidence spans.
- **Architecture:** `roberta-large` + PEFT LoRA (`RobertaForTokenClassification`).
- **This repo:** Single-marker model for **Evidence** only.
